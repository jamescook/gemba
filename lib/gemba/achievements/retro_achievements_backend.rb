...
module Gemba
  module Achievements
    ...
    #
    # All HTTP is done off the main thread via Teek::BackgroundWork (thread mode).
    #
    # Authentication flow:
    #   login_with_password  → r=login2 + password → stores token, fires :ok
    #   login_with_token     → r=login2 + token    → verifies token, fires :ok/:error
    #   ping                 → same as login_with_token using stored creds
    #
    ...
    class RetroAchievementsBackend
      ...
      def initialize(app:)
        ...
        @ra_runtime       = Gemba::RARuntime.new
      end
      ...
      def fetch_patch_data(game_id, emit_sync_done: false)
        ra_request(r: 'patch', u: @username, t: @token, g: game_id) do |json, ok|
          ...
          fetch_unlocks(game_id, raw_ach_data: raw, emit_sync_done: emit_sync_done)
        end
      end
      ...
      def fetch_unlocks(game_id, raw_ach_data: nil, emit_sync_done: false)
      ...
      # POST to dorequest.php in a background thread.
      # Calls block with (json_hash_or_nil, success_bool) on the main thread.
      def ra_request(params, &on_done)
        Teek::BackgroundWork.new(@app, params, mode: :thread) do |t, req_params|
