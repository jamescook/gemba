...
module Gemba
  # Top-level application window — a dumb container that hosts swappable frames.
  #
  # MainWindow owns the Tk app, menus, modal stack, config, and input maps.
  # It forwards all emulation concerns to the hosted frame via @frame.receive.
  # The frame communicates back via EventBus.
  class MainWindow
    include Gemba
    ...
    attr_reader :app, :config, :settings_window, :kb_map, :gp_map, :running, :scale, :frame
    ...
    def load_rom(path)
      ...
      saves = @config.saves_dir
      loaded_core = @frame.load_core(rom_path, saves_dir: saves, rom_source_path: path)
      @rom_path = path

      new_platform = @frame.platform
      ...
      @app.set_window_title("gemba \u2014 #{loaded_core.title}")
      ...
      if File.exist?(sav_path)
        @frame.receive(:show_toast, message: translate('toast.loaded_sav', name: sav_name))
      else
        @frame.receive(:show_toast, message: translate('toast.created_sav', name: sav_name))
      end
